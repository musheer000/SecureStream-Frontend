const fs = require("fs");
const content = `import React, { useEffect, useRef, useState, useCallback } from "react";
...
import {
  AlertTriangle, Shield, Wifi, WifiOff, CheckCircle2,
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(56,189,248,0.08)" }}>
                  {["Severity", "Type", "Source IP", "Target", "Description", "Time", "Action"].map(h => (
              ...
              <tbody>
                {threats.map((t) => (
                  <tr key={t.id} style={{
                    ...
                    background: t.severity === "CRITICAL" ? "rgba(239,68,68,0.06)" : t.severity === "HIGH" ? "rgba(249,115,22,0.03)" : "transparent",
...
`;
fs.writeFileSync("src/ThreatDashboard.tsx", content);
